<?php
    ...
    markupa($_GET['id']);
    $sqli = "SELECT * FROM `fp_ann` WHERE `owner` LIKE '".$_GET['id']."' AND (`allow` LIKE 'true' OR `allow` LIKE '%".cookie('User')."%') order by `id` desc";
    if ($get = mysqli_query($conn, $sqli)){
        if (0 < mysqli_num_rows($get)){
            while($nval = mysqli_fetch_array($get)){          
            annd(array('title'=>$nval['title'], 'msg'=>$nval['msg']));
            }
        }else{
