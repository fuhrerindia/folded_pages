...
">New App</h3>
    ...
    <button onClick="getbyid('nann').style.display='none'">Cancel</button>
      ...
      <?php
            if (isset($_POST['save'])){
                ...
                if ($uploaded = mysqli_query($conn, $saveann)){
                    script('notify(\'Video Saved\', \'Video is available for students. They can see it into Conferencing Apps Tab.\')');
                }else{
      ...
      ?>
    <?php
    markupadmin($_GET['id'], array("title"=>'Add App', "nid"=>'nann'));
    $sqli = "SELECT * FROM `fp_link` WHERE `batch` LIKE '".$_GET['id']."' order by `id` desc";
    if ($get = mysqli_query($conn, $sqli)){
        if (0 < mysqli_num_rows($get)){
            ...
            if (isset($_POST['delete'])){
                if (empty($_POST['sel'])){
                    alert('No Conferencing App Selected to delete');
                }else{
                    $i = 1;
                ...
                $deletesql = "DELETE FROM `fp_link` WHERE ".$selections;
                if ($deleted = mysqli_query($conn, $deletesql)){
                    alert('Selected Conferencing Apps are deleted successfully.');
                    open('');
                }else{
            ...
            while($nval = mysqli_fetch_array($get)){          
                adminaddapp($nval['url'], $nval['title'], $nval['app'], $nval['id']);
                        }
            ?>
            </form>
            <?php
        }else{
            html('No Conferencing App is available for you.');
        }
    }else{
