<?php
    ...
    if ($conn = mysqli_connect($server, $user, $pass, $db)){
        ...
        if ($run = mysqli_query($conn, $sql)){
            ...
            foreach(json_decode($data['fp_batches']) as $col){
                ...
                if ($allow === true){
    boiler('Folded Pages - Class Link &bull; TMINC', 
    ...
    markupa($_GET['id']);
    $sqli = "SELECT * FROM `fp_link` WHERE `batch` LIKE '".$_GET['id']."' AND (`allow` LIKE 'true' OR `allow` LIKE '%".cookie('User')."%') order by `id` desc";
    if ($get = mysqli_query($conn, $sqli)){
        if (0 < mysqli_num_rows($get)){
            while($nval = mysqli_fetch_array($get)){          
                addapp($nval['url'], $nval['title'], $nval['app']);
            }
        }else{
            html('No Live Video Conferencing App is linked.');
        }
    }else{
