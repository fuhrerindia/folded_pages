<?php
    ...
    if ($conn = mysqli_connect($server, $user, $pass, $db)){
        ...
        if ($run = mysqli_query($conn, $sql)){
            ...
            foreach(json_decode($data['fp_batches']) as $col){
                ...
                if ($allow === true){
    boiler('Folded Pages - Videos &bull; TMINC', 
    ...
    markupa($_GET['id']);
    $sqli = "SELECT * FROM `fp_video` WHERE `batch` LIKE '".$_GET['id']."' AND (`allow` LIKE 'true' OR `allow` LIKE '%".cookie('User')."%') order by `id` desc";
    if ($get = mysqli_query($conn, $sqli)){
        if (0 < mysqli_num_rows($get)){
            while($getv = mysqli_fetch_array($get)){
            video(array('img'=>$getv['img'], 'point'=>$getv['url'], 'title'=>$getv['title']));
            }
        }else{
            html('No Video is available for you.');
        }
    }else{
